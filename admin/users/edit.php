...
if (!$userId) {
    redirect(SITE_URL . '/admin/users/index.php', 'Invalid user ID', 'error');
}

$user = $db->fetchOne("SELECT * FROM users WHERE id = ?", [$userId]);

if (!$user) {
    redirect(SITE_URL . '/admin/users/index.php', 'User not found', 'error');
}

// Prevent editing admin users
if ($user['role'] === 'admin') {
    redirect(SITE_URL . '/admin/users/index.php', 'Cannot edit admin users', 'error');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($formData['name'])) {
        ...
        $checkEmail = $db->fetchOne("SELECT id FROM users WHERE email = ? AND id != ?", [$formData['email'], $userId]);
        if ($checkEmail) {
            $error = 'Email sudah terdaftar';
        } else {
            // Check if user_code already exists (except current)
            $checkCode = $db->fetchOne("SELECT id FROM users WHERE user_code = ? AND id != ?", [$formData['user_code'], $userId]);
            if ($checkCode) {
                ...
                if (!empty($formData['password'])) {
                    ...
                    $query = "UPDATE users SET
                              name = ?, email = ?, user_code = ?, password = ?, role = ?,
                              university_id = ?, phone = ?, address = ?
                              WHERE id = ?";
                    $params = [
                        $formData['name'],
                        $formData['email'],
                ...
                if ($db->execute($query, $params)) {
                    logActivity($currentUser['id'], 'edit_user', 'users', 'Updated user: ' . $formData['name']);
                    redirect(SITE_URL . '/admin/users/index.php', 'User berhasil diupdate!', 'success');
                } else {
                    $error = 'Gagal mengupdate user. Silakan coba lagi.';
                }
            }
        }
    }
...
$pageTitle = 'Edit Pengguna - ' . SITE_NAME;
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <div class="main-content">
        ...
        <div class="content-wrapper">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h1 class="h2 fw-bold mb-1">Edit Pengguna</h1>
